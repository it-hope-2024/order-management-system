...
<div
    class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800 flex flex-col justify-between min-h-[400px]">
    <div>
        ...
        <div class="pt-6">
            <a href="#"
                class="text-lg font-semibold leading-tight text-gray-900 hover:underline dark:text-white block min-h-[48px]">
                {{ $product->getTranslation('name', app()->getLocale()) }}
            </a>

            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                {{ __('messages.in-stock') }}:
                <span id="product-stock-{{ $product->id }}"
                    class="font-semibold {{ $product->stock > 0 ? 'text-green-600' : 'text-red-600' }}">
    ...
    <div class="mt-auto">
        @if ($product->stock > 0)
            <div class="mt-4 flex flex-col items-center gap-3">
                ...
                @auth
                    <button type="button" id="add-to-cart-btn-{{ $product->id }}"
                        ...
                        <svg class="-ms-2 me-2 h-5 w-5 inline" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
